...
export const NoticeTransactionWentTrue = ({ device, userData }) => {
  const [stripeTransactions, setStripeTransactions] = useState([]);

  const payment_intent = new URLSearchParams(window.location.search).get(
    "payment_intent"
  );
  console.log("🚀 ~ file: NoticeTransactionWentTrue.js:13 ~ NoticeTransactionWentTrue ~ payment_intent", payment_intent)

  const clientSecret = new URLSearchParams(window.location.search).get(
    "payment_intent_client_secret"
  );
  console.log("🚀 ~ file: NoticeTransactionWentTrue.js:18 ~ NoticeTransactionWentTrue ~ clientSecret", clientSecret)
